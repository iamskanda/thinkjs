                ...
                <div class="col-12">
                    ...
                    <div class="card">
                        <div class="card-header">
                            ...
                            <div class="col-sm-9">
                                <h3 class="card-title">Role Management Details</h3>
                            </div>
...
<div class="modal fade update-master">
    <div class="modal-dialog modal-dialog-centered">
        <div class="modal-content">
            <div class="modal-header" style="background-color:#439bc1;padding:10px;">
                <h2 class="modal-title" id="exampleModalLabel" style="color:white;font-weight: 600;font-size: 18px;">Update Master</h2>
                <button type="button" class="close" id="clos" data-dismiss="modal" aria-label="Close">
                    <span aria-hidden="true">&times;</span>
                </button>
            </div>
...
<script>
    ...
    $('#master-data').submit(function() {
        ...
        $.ajax({
            type: "POST",
            url: '<?php echo base_url("add-master-data") ?>',
            data: formData,
            contentType: false,
            processData: false,
    ...
    $(document).on('click', '#updateCountryBtn', function() {
        ...
        $.ajax({
            url: "<?php echo base_url("update-master-data") ?>",
            type: "POST",
            data: {
                id: id
            },
            success: function(response) {
                ...
                if (response.result == 1) {
                    ...
                    $(".master_name_edit").val(response.message.master_name);
        ...
        swal({
            ...
            function(isConfirm) {
                if (isConfirm) {
                    ...
                    $(".sweet-overlay").hide();
                    $.ajax({
                        url: "<?php echo base_url("delete-master") ?>",
                        type: "POST",
                        data: {
                            id: id
                        },
    ...
    $(document).on('click', '#subMasterCountryBtn', function() {
        ...
        window.location = "<?php echo base_url("sub-master") ?>?id=" + id;
    ...
    $('#library-table').DataTable({
        ...
        "searching": true,
        "lengthChange": false,
        "ajax": "<?php echo base_url('show-all-master-data') ?>",
        "dom": "lBfrtip",
        "stateSave": true,
        "info": true,
        "bInfo": true,

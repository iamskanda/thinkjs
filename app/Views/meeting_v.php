    ...
    <div class="modal-dialog modal modal-dialog-centered">
        <div class="modal-content">
            <div class="" style="background-color:#439bc1;padding:10px;">
                ...
                <div class="row">
                    <div class="col-sm-9">
                        <h2 class="modal-title" id="exampleModalLabel" style="color:white;font-weight: 600;font-size: 18px;">Add New Meeting</h2>
                    </div>
                    <div class="col-sm-3">
                ...
                <form id="update-master-data">
                    ...
                    <div class="row">
                        <div class="col-sm-12">
                            <div class="form-group">
                                <label for="">Reason</label>
                                <input type="text" class="form-control" id="reason_u" name="reason_u">
                            </div>
                        </div>
                    </div>
...
<script>
    ...
    $('#sub-master-data').submit(function() {
        ...
        $.ajax({
            type: "POST",
            url: '<?php echo base_url("add-new-meeting") ?>',
            data: formData,
            contentType: false,
            processData: false,
        ...
        swal({
            ...
            function(isConfirm) {
                if (isConfirm) {
                    ...
                    $(".sweet-overlay").hide();
                    $.ajax({
                        url: "<?php echo base_url("delete-meeting") ?>",
                        type: "POST",
                        data: {
                            id: id
                        },
    ...
    $(document).on('click', '#updateCountryBtn', function() {
        ...
        $.ajax({
            url: "<?php echo base_url("update-meeting-data") ?>",
            type: "POST",
            data: {
                id: id
            },
            success: function(response) {
                ...
                if (response.result == 1) {
                    ...
                    $("#link_u").val(response.message.link);
                    $("#reason_u").val(response.message.Reason);
    ...
    $('#update-master-data').submit(function(e){  
        ...
        $.ajax({
            type   : 'post',
            url    : '<?php echo base_url("update-meeting-data-list")?>',
            data   : formdata,
            contentType: false,
            processData: false,					  
            success:function(response){
    ...
    $('#library-table').DataTable({
        ...
        "searching": true,
        "lengthChange": false,
        "ajax": "<?php echo base_url('show-all-meeting-data') ?>",
        "dom": "lBfrtip",
        "stateSave": true,
        "info": true,
        "bInfo": true,
